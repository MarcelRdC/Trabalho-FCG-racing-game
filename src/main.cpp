...
void TextRendering_ShowSpeed(GLFWwindow* window, float speed);
...
int main(int argc, char* argv[])
{
    ...
    ObjModel carmodel("../../data/sportcar.obj");
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        // Atualização da câmera com base na posição do carro
        if(third_cow_view)camera_position_c=glm::vec4(g_PosX,g_PosY+1.5f,g_PosZ,1.0f)-camera_view_vector;
        ...
        g_PosX=g_PosX+((camera_view_vector[0])/norm(camera_view_vector))*0.5f*speed*delta_t;
        ...
        if(move_camera_A){
            if(speed>0){
                g_AngleY = g_AngleY +3*delta_t;
                g_CameraTheta = g_CameraTheta +3*delta_t;
            }
            if(speed<0){
                g_AngleY = g_AngleY -3*delta_t;
                g_CameraTheta = g_CameraTheta -3*delta_t;
        ...
        DrawVirtualObject("WheelRL_Circle.004");
        DrawVirtualObject("WheelRR_Circle.002");
        DrawVirtualObject("WheelFL_Circle.001");
        DrawVirtualObject("WheelFR_Circle");
        ...
        model = Matrix_Translate(oponente2.x+3.0f, oponente2.y, oponente2.z)
                * Matrix_Rotate_Z(g_AngleZ)  // TERCEIRO rotação Z de Euler
                * Matrix_Rotate_Y(M_PI+g_AngleY)  // SEGUNDO rotação Y de Euler
                * Matrix_Rotate_X(g_AngleX); // PRIMEIRO rotação X de Euler
        glUniformMatrix4fv(g_model_uniform, 1 , GL_FALSE , glm::value_ptr(model));
        ...
        for (grid=0; grid <52; grid++){
            ...
            model = Matrix_Translate(offset_x,0.0f,offset_z)
                * Matrix_Rotate_Y( M_PI_2+temp_angle);
        glUniformMatrix4fv(g_model_uniform, 1 , GL_FALSE , glm::value_ptr(model));
        glUniform1i(g_object_id_uniform, FENCE);
        DrawVirtualObject("wall_End");
        }
...
void TextRendering_ShowSpeed(GLFWwindow* window, float speed)
{
    ...
    char buffer[80];
    snprintf(buffer, 80, "%.0f", speed);

    TextRendering_PrintString(window, buffer, -1.0f+pad, -1.0f+2*pad, 4.0f);
}
